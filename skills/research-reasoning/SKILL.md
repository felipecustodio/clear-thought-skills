---
name: research-reasoning
description: Use this skill for research.
license: MIT
compatibility: Requires Python 3.12+ and uv when running bundled scripts.
---

## Use When

- The user asks for research reasoning
- You need to perform research

## Workflow

1. Determine the parameters for the task.
2. Execute the necessary steps.
3. Run the script `scripts/research_reasoning.py` to validate or compute.
3. Format the final output for the user.

## Outputs

- A formatted response with reasoning and conclusions.

## Scripts

- `scripts/research_reasoning.py` - source matrix formatter

## Gotchas

- Ensure all required parameters are provided.
